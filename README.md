...
# 2026 Update: Architektur, Wartungstool & UI v2 (additiv)

> **Hinweis (Legacy):** Bestehende Dokumentation/Abschnitte in dieser README sind weiterhin gültig, können aber für einzelne Seiten/Module **Legacy** sein.

> Wir löschen nichts – neue Standards werden unten ergänzt und Alt-Verhalten wird markiert.
...
- **Audit:** `core_audit_log` (ISO-tauglich / revisionssicher)
...
## UI v2: Design System & Templates

**Wichtig:** `main.css` ist in der Migration **Legacy** und wird **nicht mehr aktiv erweitert**.

Neue/umgebaute Seiten nutzen ausschließlich das **UI-v2 Template** (Klassen `ui-*`).

Alt-Seiten werden schrittweise migriert (Desktop-first, später Tablet/Smartphone).

➡️ **Guide/Template:** `docs/UI_V2_GUIDE.md` (neu, 2026-02-27)

### Status der UI v2 Migration (Wartungstool)

- `wartung.dashboard` → UI v2 (KPI links + Insights rechts) ✅
- `wartung.uebersicht` → UI v2 (KPI + Filterbar + Tabellen mit Wrap) ✅
- `wartung.admin_punkte` → UI v2 (2-Spalten: Form/Tools links, Liste/Audit rechts) ✅
- `wartung.punkt` → UI v2 **noch offen** (TODO)

> **Hinweis:** Einige alte Styles (z.B. `ticket.css` / Legacy-Layouts) werden schrittweise ersetzt.

> Bitte **keine** neuen Features in Legacy-CSS einbauen.

## Wartungslogik: „Bald fällig“ pro Wartungspunkt

### Tabelle: `wartungstool_wartungspunkt`

Zweck: Wartungsplan pro Asset (zeit- oder produktivstundenbasiert).

- `plan_interval`: Stunden (bei `produktiv`) oder Stunden/Zeitraum (bei `zeit`; Interpretation liegt im Code)
...
### Bald-fällig Schwelle (konfigurierbar pro Wartungspunkt)

- `soon_hours` (DOUBLE, NULL): absolute Restschwelle in Stunden (**Gewinner**, wenn nicht NULL)
- `soon_ratio` (DOUBLE, NULL): relative Restschwelle 0..1

  Beispiel: `0.10` ⇒ „bald“, wenn `rest/interval <= 0.10`
- Wenn beide NULL: Default-Schwelle wird im Code verwendet (aktuell **0.20**)

### Status-Logik (Soll-Zustand, zu vereinheitlichen)

1. **Überfällig:** `rest < 0`
2. **Bald fällig:**

   - wenn `soon_hours` gesetzt → `rest <= soon_hours`

   - sonst wenn `soon_ratio` gesetzt → `rest/interval <= soon_ratio`

   - sonst → Default `0.20`
3. **OK:** sonst

> TODO (bereits identifiziert): Dashboard & Übersicht sollen die exakt gleiche Berechnung nutzen (zentralisieren).
